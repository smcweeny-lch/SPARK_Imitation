...
available_Affect.Sharing <- setdiff(available_Affect.Sharing,  which(names(data) %in% items_list[["Social.Eng"]]))
...
for(i in 1:niter){
  ...
  while(alpha_val < real_reli[names(real_reli) == construct] - .15){
    ...
    temp_available <- setdiff(available_Affect.Sharing, Imi.col_idx)
    
    
    affect.sharing.col_idx <- sample(temp_available, size = len)
  ...
  while(alpha_val < real_reli[names(real_reli) == construct] - .15){
    ...
    social.eng.overlap.items <- sample(affect.sharing.col_idx, size = 3)
    
    temp_available <- setdiff(available_Social.Eng, social.eng.overlap.items)
    
    
    
    social.eng.col_idx <- sample(temp_available, size = len - 3)
    
    social.eng.col_idx <- c(social.eng.col_idx, social.eng.overlap.items)
  ...
  alpha_val <- 0
  while(alpha_val < real_reli[names(real_reli) == paste0(construct, ".11")] - .15){
    
    temp_available <- setdiff(available_Symbolism , Imi.col_idx) 
    
    symbolism.11.col_idx <- sample(temp_available, size = len)
  ...
  alpha_val <- 0
  while(alpha_val < real_reli[names(real_reli) == paste0(construct, ".12")] - .15){
    
    temp_available <- setdiff(available_Symbolism , Imi.col_idx) 
    
    symbolism.12.col_idx <- sample(temp_available, size = len)
  ...
  alpha_val <- 0
  while(alpha_val < real_reli[names(real_reli) == paste0(construct, ".21")] - .15){
    
    temp_available <- setdiff(available_Symbolism , Imi.col_idx) 
    
    symbolism.21.col_idx <- sample(temp_available, size = len)
  ...
  alpha_val <- 0
  while(alpha_val < real_reli[names(real_reli) == paste0(construct, ".22")] - .15){
    
    temp_available <- setdiff(available_Symbolism , Imi.col_idx) 
    
    symbolism.22.col_idx <- sample(temp_available, size = len)
...
# saveRDS(nulls, "./code/Empirical_priors/R_and_P_pseudo_construct_all_constraints_4_22_25.rds")
# saveRDS(pseudo_items_list, "./code/Empirical_priors/R_and_P_pseudo_construct_items_all_constraints_4_22_25.rds")
# saveRDS(relis_df, "./code/Empirical_priors/R_and_P_pseudo_construct_reliabilities_all_constraints_4_22_25.rds")

saveRDS(nulls, "./code/Empirical_priors/R_and_P/output/R_and_P_pseudo_construct_all_constraints_8_11_25.rds")
saveRDS(pseudo_items_list, "./code/Empirical_priors/R_and_P/output/R_and_P_pseudo_construct_items_all_constraints_8_11_25.rds")
saveRDS(relis_df, "./code/Empirical_priors/R_and_P/output/R_and_P_pseudo_construct_reliabilities_all_constraints_8_11_25.rds")
