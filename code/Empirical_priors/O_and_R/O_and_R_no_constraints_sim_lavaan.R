# This is the supplemental script for generating the pseudo-constructs (and relations among them)
# for Ornitz & Ritvo (1968). The data structures are from many sources, and need some explaining
...
# This was modified from simulate_pseudoconstructs_O_and_R_reliability_constraints_sim_lavaan.R on April 28, 2025
...
for(i in 1:niter){
  ...
  len <- length(items_list[[construct]])
    ...
    self.nonself.col_idx <- sample(available_Self.Nonself, size = len)
  ...
  len <- length(items_list[[construct]])
    ...
    rc1.col_idx <- sample(available_RC.F1, size = len)
  ...
  len <- length(items_list[[construct]])
    ...
    rc2.col_idx <- sample(available_RC.F2, size = len, replace = T)
  ...
  len <- length(items_list[[construct]])
    ...
    VL2.F1.col_idx <- sample(available_VL2.F1, size = len)
  ...
  len <- length(items_list[[construct]])
    ...
    VL2.F2.col_idx <- sample(available_VL2.F2, size = len)
