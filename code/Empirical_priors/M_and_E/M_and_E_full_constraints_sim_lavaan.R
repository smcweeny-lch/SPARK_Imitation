# This script was modified from "simulate_pseudoconstructs_M_and_E.R 
# on April 21, 2025 after discovering that sim_betas.R used lm slopes that were not
...
# In addition, it appears that a "full_constraints" version was never run during the first batch (March 27) - 
# the simulation in  "simulate_pseudoconstructs_M_and_E.R" did not restrict overlapping items from "available." 
...
# no need to exclude imitation items, as they were not voted on
...
for(i in 1:niter){
  ...
  construct <- "Imitation"
  # The line updates from "reliability constraints only" to "full" constraints
  len <- length(items_list_lean[[construct]])
  ...
  while(alpha_val < real_reli_lean[names(real_reli_lean) == construct] - .15){
    ...
    temp_available <- setdiff(available_Imitation, soc.skill.col_idx)
...
construct_items_list[["Social.Skills"]][1,]
...
# saveRDS(nulls, "./code/Empirical_priors/M_and_E_pseudo_construct_full_constraints_4_21_25.rds")
# saveRDS(pseudo_items_list, "./code/Empirical_priors/M_and_E_pseudo_construct_items_full_constraints_4_21_25.rds")
# saveRDS(relis_df, "./code/Empirical_priors/M_and_E_pseudo_construct_reliabilities_full_constraints_4_21_25.rds")

saveRDS(nulls, "./code/Empirical_priors/M_and_E_pseudo_construct_full_constraints_8_11_25.rds")
saveRDS(pseudo_items_list, "./code/Empirical_priors/M_and_E_pseudo_construct_items_full_constraints_8_11_25.rds")
saveRDS(relis_df, "./code/Empirical_priors/M_and_E_pseudo_construct_reliabilities_full_constraints_8_11_25.rds")
