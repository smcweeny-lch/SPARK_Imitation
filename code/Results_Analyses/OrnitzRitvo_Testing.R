...
# nulls2 <- readRDS("./code/Empirical_priors/O_and_R/output/O_and_R_pseudo_construct_all_constraints_4_21_25.rds")
...
# adjusted based on $est not $std.all - barely changed (< .01) but worth updating
# OR.1968.syntax <- '
# Self.Nonself ~ b1*Perceptual.Inconstancy
# Imitation ~ b2*Perceptual.Inconstancy
# RC.F1 ~ b3*Imitation + b5*Self.Nonself + b7*Perceptual.Inconstancy
...
saveRDS(OR.mod, "./code/Results_Analyses/lavaan_models/O_and_R/OR_mod.rds")

summary(OR.mod, standardized = T, fit.measures = T)
...
# Imi --> RC.F2 .116 vs RC F1 
 ...
 # didn't comment out $est, super minor changes (<.005)
OR.1968.syntax.22 <- '
Self.Nonself ~ b1*Perceptual.Inconstancy
Imitation ~ b2*Perceptual.Inconstancy
...
saveRDS(OR.mod.22, "./code/Results_Analyses/lavaan_models/O_and_R/OR_mod_22.rds")
...
saveRDS(OR.mod.12, "./code/Results_Analyses/lavaan_models/O_and_R/OR_mod_12.rds")
...
saveRDS(OR.mod.21, "./code/Results_Analyses/lavaan_models/O_and_R/OR_mod_21.rds")
...
graph_data$edges$label[1:8]  <- format(round(parameterestimates(OR.mod, standardized = T)[c(15:17, 19, 21, 18, 20, 22),]$est, 2), nsmall = 2)
