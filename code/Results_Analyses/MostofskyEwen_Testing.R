...
estimates <- parameterEstimates(ME.mod)

t_b1 <- estimates$z[estimates$lhs == "b1_adj"]
1-pnorm(t_b1)

t_b2 <- estimates$z[estimates$lhs == "b2_adj"]
1-pnorm(t_b2)

t_phi <- estimates$z[estimates$lhs == "phi_adj"]
1-pnorm(t_phi)


r_prior <- 0.228

r_prior_z <- fisherz(r_prior)
r_z <- cor.test(data2$Social.Skills, data2$Motor.Skills)$estimate

z_stat <- (r_z - r_prior_z) / (1/sqrt(19053 -3))
1 - pnorm(z_stat)
